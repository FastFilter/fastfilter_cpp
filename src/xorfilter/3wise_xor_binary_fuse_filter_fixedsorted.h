#ifndef THREEWISE_XOR_BINARY_FUSE_FILTER_XOR_FILTER_FIXEDSORTED_H_
#define THREEWISE_XOR_BINARY_FUSE_FILTER_XOR_FILTER_FIXEDSORTED_H_
#include "xor_binary_fuse_filter.h"
namespace xorbinaryfusefilter_fixedsorted {
...
Status XorBinaryFuseFilter<ItemType, FingerprintType, HashFamily>::AddAll(
  ...
  while (true) {
    ...
    // non-counting bucket sort:
    // we estimate the bucket sizes
    // and process in most likely 2 phases: the first phase about 90%, and then the remainder
    size_t *sizes = new size_t[1 << blockBits]();
    size_t *offsets = new size_t[1 + (1 << blockBits)]();
    for (int i = 0, x = 0; i < 1 << blockBits; i++, x += size / (1 << blockBits)) {
      offsets[i] = x;
    }
    offsets[1 << blockBits] = size;
    for (size_t p = start; p < end; p++) {
      uint64_t k = keys[p];
      uint64_t hash = (*hasher)(k);
      int bucket = hash >> (64 - blockBits);
      reverseOrder[offsets[bucket] + sizes[bucket]] = hash;
      sizes[bucket]++;
      if (sizes[bucket] + offsets[bucket] >= offsets[bucket+1]) {
        // process what we have so far
        for (int i = 0; i < 1 << blockBits; i++) {
          for (size_t j = 0; j < sizes[i]; j++) {
            uint64_t hash = reverseOrder[offsets[i] + j];
